...
const VIDEO_CARDS = [
  {
    ...
    tag: "Memory storage",
    title: "Groq extraction, HydraDB write",
    description:
      "Memories are extracted by Groq's gpt-oss-120b in real time and written into the graph as Memory nodes, linked to their Session and to the Entity they're about.",
  },
  {
    videoUrl:
    ...
    description:
      "Every query scopes to the requesting user's id, both in the graph traversal and at the API layer. Full control over what's stored and when it's pruned.",
  },
];
...
const ICON_CARDS = [
  {
    ...
    description:
      "Every memory saved, whether from Claude Code, Cursor, Codex, or Antigravity, lands in the same graph. A live dashboard polls every 3 seconds, animating new memories as they arrive.",
  },
  {
    glyph: "◎",
    anim: "semantic",
    ...
    <section className="py-24 md:py-32 px-6 border-t" style={{ borderColor: "var(--rule)" }}>
      <div className="max-w-6xl mx-auto" ref={ref}>
        <div className="flex justify-between items-baseline mb-12">
          ...
          <span className="hidden md:block text-sm" style={{ color: "var(--ink-dim)", fontFamily: "var(--font-mono)" }}>HydraDB · Jina AI · Groq</span>
        </div>
          ...
          {VIDEO_CARDS.map((card, i) => (
            ...
            >
              ...
              <div className="p-6 md:p-8">
                <span className="text-xs tracking-widest uppercase mb-3 block" style={{ color: "var(--ink-dim)", fontFamily: "var(--font-mono)" }}>
                  {card.tag}
                </span>
                <h3
                  className="text-xl md:text-2xl mb-3 tracking-tight"
                  style={{ fontFamily: "var(--font-display)", color: "var(--ink)" }}
                >
                  {card.title}
                </h3>
                <p className="text-sm leading-relaxed" style={{ color: "var(--ink-dim)" }}>{card.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
            ...
            >
              <div className="aspect-video overflow-hidden flex items-center justify-center relative">
                ...
                <span style={{ position: "absolute", bottom: 20, left: 24, fontSize: 10, letterSpacing: "0.18em",
                  textTransform: "uppercase", color: card.color, opacity: 0.6, fontWeight: 600, fontFamily: "var(--font-mono)" }}>
                  {card.tag}
                </span>
              </div>
              <div className="p-6 md:p-8">
                <span className="text-xs tracking-widest uppercase mb-3 block" style={{ color: card.color, fontFamily: "var(--font-mono)" }}>
                  {card.tag}
                </span>
                <h3
                  className="text-xl md:text-2xl mb-3 tracking-tight"
